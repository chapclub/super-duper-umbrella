...
api.post('/touch', (req, res) => {
  if (!req.body || !req.body.touch) {
    res.status(400).send({ err: 'Bad body sent. Expected: touch' });
  }
  const touch = req.body.touch;
  const { err, touch } = touchController.postTouch(touch);
  if (err) res.status(500).send({ err: err });
  res.status(201).send({ touch: touch });
});
